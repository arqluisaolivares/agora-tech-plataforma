...
def money_val(v):
    """Convierte valores tipo 86190000, '$86.190.000' o '$86,190,000' a entero."""
    try:
...
def get_money(r, num_col, text_col=None):
    """Primero intenta la columna numérica; si viene en cero, usa la columna texto."""
    n = money_val(r.get(num_col, 0))
    if n == 0 and text_col:
        n = money_val(r.get(text_col, 0))
    return n
...
def sidebar():
    ...
    with st.sidebar:
        ...
        nav_todos=[("📊","Dashboard"),("🧮","Nueva Cotización"),
...
def pg_edificios():
    ...
    with col_list:
        ...
        for _, r in dff.iterrows():
            tn = get_money(r, "totalNum", "total")
            total = fc(tn)
            c24n = get_money(r, "c24Num", "cuota24")
            c36n = get_money(r, "c36Num", "cuota36")
            ...
            st.markdown(f"""<div style='background:white;border:1px solid #E3EAF3;border-radius:12px;padding:16px;margin-bottom:12px;box-shadow:0 1px 6px rgba(4,17,30,.05)'>
              <div style='font-family:Sora,sans-serif;font-size:15px;font-weight:700;color:#04111E;margin-bottom:4px'>{str(r["nombre"])[:40]}</div>
    ...
    with col_detail:
        ...
        if seleccionado:
            ...
            with tab1:
                ...
                total_n = get_money(r, "totalNum", "total")
                c24_n = get_money(r, "c24Num", "cuota24")
                c36_n = get_money(r, "c36Num", "cuota36")
                c1.metric("Valor Total", fc(total_n))
                c2.metric("Cuota 24m", fc(c24_n))
                c3.metric("Cuota 36m", fc(c36_n))
            ...
            st.info("👈 Haz clic en 'Ver detalle completo' de cualquier edificio de la lista")
...
if not st.session_state.logged_in:
    ...
    elif pg=="Proyectos":           pg_edificios()
