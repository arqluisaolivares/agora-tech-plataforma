...
# ══════════════════════════════════════════
# IA — GEMINI  ← CAMBIOS APLICADOS AQUÍ
# ══════════════════════════════════════════
...
# ← CAMBIO 1: modelos actualizados, eliminado gemini-1.5-flash
GEMINI_MODELS = ["gemini-2.0-flash", "gemini-1.5-flash-002", "gemini-1.5-pro-002"]
...
def ask_ai(q, ctx=""):
    ...
    try:
        ...
        # ← CAMBIO 2: fallback a modelo actualizado, eliminado gemini-1.5-flash
        try:
            genai.configure(api_key=key)
            m = genai.GenerativeModel("gemini-1.5-flash-002", system_instruction=AI_SYSTEM)
            prompt = f"DATOS CRM:\n{ctx[:20000]}\n\nSOLICITUD:\n{q}" if ctx else q
            r = m.generate_content(prompt)
            return r.text
        ...
        with st.expander(f"🏢  {r['nombre']}   {badge(est)}   {total}   {r.get('comercial','')}",expanded=False):
            ...
            with tab_ejec:
                ...
                if grupo_actual in ["Ejecución","Posventa"]:
                    ...
                    st.info("Este proyecto aún está en etapa comercial.")
...
def pg_encuesta():
    ...
    with st.form("encuesta_form"):
        ...
        incidentes=st.text_area("¿Han ocurrido robos o incidentes de seguridad en la copropiedad?",
            placeholder="Describir incidentes relevantes...",height=80)

        st.markdown("---")
        st.markdown("**Comunidad**")
        adultos_mayores=st.text_area("¿El edificio cuenta con adultos mayores o personas con discapacidad?",
            placeholder="Ej: Hay varios adultos mayores en los pisos altos...",height=80)
        ...
        with c2:
            costo_vig=st.number_input("¿Cuál es el costo al mes? ($)",min_value=0,value=0,step=100_000,format="%d") if tiene_vigilancia=="Sí" else 0
        vigencia=st.text_input("¿Hasta cuándo está vigente el contrato de vigilancia?",placeholder="Ej: Noviembre 2026")
        servicios_adicionales=st.text_area("¿El servicio de vigilancia presta servicios adicionales?",
            placeholder="Ej: Sí, el vigilante apoya con paquetes...",height=70)

        st.markdown("---")
        st.markdown("**Proceso de selección**")
        terminos=st.text_area("¿Tienen algunos términos de selección para la automatización?",
            placeholder="Ej: Necesitan 3 cotizaciones, requieren presentación en asamblea...",height=70)
        ...
        if submitted:
            ...
            if edificio_sel!="— Nuevo prospecto (sin edificio aún) —":
                ...
                nota_hist=f"Encuesta completada. Contacto: {contacto} ({rol}). Etapa: {etapa_decision}."
                agregar_historial(edificio_sel,"lead",nota_hist,st.session_state.user["nombre"])
                st.success(f"✅ Encuesta guardada en **{edificio_sel}**")
            if analizar_ia and ai_activa():
                prompt=f"""Analiza este prospecto para Ágora Tech Colombia y genera estrategia de venta:
EDIFICIO: {nombre_edif} | Contacto: {contacto} ({rol})
Etapa: {etapa_decision} | Impulsor: {impulsor or "No especificado"}
Vigilancia: {"SÍ — "+fc(costo_vig)+"/mes hasta "+vigencia if tiene_vigilancia=="Sí" and costo_vig else "NO"}
Incidentes: {incidentes or "Ninguno"} | Adultos mayores: {adultos_mayores or "No reportado"}
Términos: {terminos or "Sin términos"}
...
## PRÓXIMOS PASOS (esta semana)"""
                with st.spinner("Analizando con IA..."): analisis=ask_ai(prompt)
                st.markdown("---")
                ...
                if edificio_sel!="— Nuevo prospecto (sin edificio aún) —":
                    agregar_historial(edificio_sel,"lead",f"Análisis IA: {analisis[:200]}...",st.session_state.user["nombre"])
            elif analizar_ia and not ai_activa():
                st.warning("Encuesta guardada. Para el análisis IA, ve a ⚙️ Configuración.")
...
def pg_configuracion():
    ...
    if ai_ok:
        modelo=st.session_state.get("gemini_modelo","gemini-2.0-flash")
        st.markdown(f'<div class="al-g">✅ <b>IA activa</b> — Gemini ({modelo})</div>',unsafe_allow_html=True)
    else:
    ...
    st.markdown("""<div style='background:#F6F9FC;border:1px solid #E3EAF3;border-radius:10px;padding:14px 18px;margin-bottom:16px;font-size:13px;color:#4A6580'>
      <b>Tu API Key de Gemini:</b><br>
      1. Ve a <b>aistudio.google.com/app/apikey</b><br>
      2. Haz clic en <b>Create API Key</b> → selecciona proyecto agora-tech-personal<br>
      3. Copia la key (empieza con AIzaSy...) y pégala abajo
    </div>""",unsafe_allow_html=True)
    ...
    with st.form("config_ia_form"):
        ...
        if st.form_submit_button("⚡ Activar IA con Gemini",use_container_width=True):
            if not nueva_key.strip(): st.error("Pega la API Key")
                ...
                if ok: st.success("✅ ¡Gemini activo!"); st.balloons(); st.rerun()

    if es_g:
        st.markdown("---")
        st.markdown("#### 📋 Configuración permanente en Streamlit Cloud")
        st.markdown("Para que todos tengan IA sin configurar nada cada vez:")
        st.code('GEMINI_API_KEY = "AIzaSy-tu-key-aqui"',language="toml")
        st.markdown("Ve a tu app en Streamlit Cloud → **⋮** → **Settings** → **Secrets** → pega eso → **Save**")
        st.markdown("---")
...
def pg_auditoria():
    ...
    with c2:
        if st.button("🤖 Análisis estratégico profundo",use_container_width=True):
            with st.spinner("Analizando..."): r=ask_ai("Top 5 proyectos más cercanos al cierre con acción específica. Patrón de rechazos. Plan 7 días.",df.to_string(max_rows=84))
            st.markdown(r)
