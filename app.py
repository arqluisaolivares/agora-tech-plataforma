...
Texto plano sin markdown."""
    ...
    with col2:
        ...
        if val!="El correo generado aparecerá aquí...":
            st.download_button("📋 Descargar",data=val,file_name=f"correo_{edif_sel[:20] if edif_sel!='— Seleccionar —' else 'agora'}.txt")
...
def pg_configuracion():
    ...
    with st.form("config_ia_form"):
        nueva_key = st.text_input(
            ...
            help="La key queda guardada en esta sesión del navegador"
        )
        guardar = st.form_submit_button("⚡ Activar IA", use_container_width=True)

        if guardar:
            if not nueva_key or len(nueva_key.strip()) < 10:
                st.error("Pega una API Key válida (debe empezar con gsk_...)")
            else:
                with st.spinner("Verificando la key..."):
                    ok = activar_ia(nueva_key)
                if ok:
                    st.success("✅ ¡IA activada correctamente! Ya puedes usar correos y el asistente.")
                    st.balloons()
                    st.rerun()
