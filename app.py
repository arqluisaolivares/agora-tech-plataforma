...
# ═══════════════════════════════════════════
# CONSTANTES DE IA — Groq (Llama 3.1 70B · Gratuito)
# ═══════════════════════════════════════════
...
def init_gemini_auto():
    """Inicializar Groq IA desde st.secrets automáticamente."""
    if st.session_state.gemini_model:
        return True
    # Intentar cargar key desde secrets de Streamlit Cloud
    key = ""
    try:
...
def ask_gemini(q, ctx=""):
    client = st.session_state.gemini_model
    if not client:
        return "⚠️ La IA no está configurada. Ve a Configuración y agrega tu API Key de Groq (gratis en console.groq.com)."
    try:
    ...
    with st.sidebar:
        ...
        if ai_ok:
            ...
            <div style='background:rgba(0,200,150,.1);border:1px solid rgba(0,200,150,.2);
              ...
              <div style='font-size:10px;color:rgba(255,255,255,.4);margin-top:1px'>Llama 3.1 · Groq</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            ...
            # Campo para ingresar key manualmente (cualquier usuario puede activarla)
            gkey_sb = st.text_input("API Key Groq", type="password",
                                     placeholder="gsk_...",
                                     key="gkey_sb",
                                     help="Ingresa la API Key de Groq para activar la IA")
            if gkey_sb:
