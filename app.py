...
Diseño corporativo premium · Claude IA (Anthropic) · Gestión de usuarios
"""
...
# ═══════════════════════════════════════════
# CONSTANTES DE IA — Claude (Anthropic)
# ═══════════════════════════════════════════
...
# ═══════════════════════════════════════════
# IA CLAUDE (Anthropic) — cargado de secrets de Streamlit
# ═══════════════════════════════════════════
...
def init_gemini_auto():
    """Intenta inicializar Claude AI desde st.secrets automáticamente."""
...
def init_gemini_key(key: str):
    """Inicializar Claude AI con una key manual."""
    try:
        client = anthropic.Anthropic(api_key=key)
        r = client.messages.create(
            model="claude-haiku-4-5",
        ...
        if r:
            st.session_state.gemini_model = client
            st.session_state["ai_key"] = key
            st.success("✅ IA activa — Claude (Anthropic)")
            return True
...
def ask_gemini(q, ctx=""):
    ...
    if not client:
        return "⚠️ La IA no está configurada. Ve a Configuración y agrega tu API Key de Claude (Anthropic)."
    try:
        prompt = f"DATOS CRM:\n{ctx[:20000]}\n\nSOLICITUD:\n{q}" if ctx else q
        r = client.messages.create(
            model="claude-haiku-4-5",
...
def pg_correos():
    ...
    if not st.session_state.gemini_model:
        st.markdown('<div class="al-r">⚠️ <b>La IA no está configurada.</b> Luisa debe ir a Configuración y agregar la API Key de Claude (Anthropic). Es gratis en console.anthropic.com</div>', unsafe_allow_html=True)
...
def pg_asistente():
    ...
    if not st.session_state.gemini_model:
        st.markdown('<div class="al-r">⚠️ <b>La IA no está configurada.</b> Ve a Configuración y agrega la API Key de Claude (console.anthropic.com — es gratis).</div>', unsafe_allow_html=True)
        return
...
def pg_configuracion():
    ...
    st.markdown("#### 🤖 Configurar IA — Claude (Anthropic)")
    st.markdown('<div class="al-g">✅ La IA usa <b>Claude de Anthropic</b> — completamente gratuito para el uso de Ágora Tech. No necesitas tarjeta de crédito.</div>', unsafe_allow_html=True)

    ai_ok = st.session_state.gemini_model is not None
    st.markdown(f"**Estado actual:** {'🟢 IA activa y funcionando' if ai_ok else '🔴 IA no configurada'}")

    with st.form("claude_form"):
        gkey = st.text_input("API Key de Claude (Anthropic)", type="password",
                              placeholder="sk-ant-api03-...",
                              help="Obtén tu key gratis en: console.anthropic.com")
    ...
    st.markdown("---")
    st.markdown("#### 📋 Cómo obtener la API Key de Claude (gratis, 2 minutos)")
    st.markdown("""
    1. Abre **[console.anthropic.com](https://console.anthropic.com)**
    2. Inicia sesión o crea cuenta con tu email
    3. Clic en **"API Keys"** en el menú izquierdo
    4. Clic en **"Create Key"** → ponle nombre `agora-tech`
    5. Copia la key (empieza con `sk-ant-...`)
    """)

    st.markdown("---")
    st.markdown("#### 📋 Para que la IA funcione siempre (configuración permanente en Streamlit Cloud)")
    st.markdown("""
    1. Ve a tu app en **share.streamlit.io**
    2. Clic en **⋮ (tres puntos)** → **Settings** → **Secrets**
    3. Pega esto exactamente:
    """)
    st.code('ANTHROPIC_API_KEY = "sk-ant-TU-KEY-AQUI"', language="toml")
    st.markdown("""
    4. Clic **Save** — la app se reinicia en 1 minuto
    5. Desde ese momento **todos los usuarios** tienen IA activa automáticamente
    """)
