...
Diseño corporativo premium · IA Gratuita (Groq) · Gestión de usuarios
"""
...
# ═══════════════════════════════════════════
# CLIENTE GROQ GLOBAL — compartido entre TODOS los usuarios
# st.cache_resource crea UNA instancia para toda la app
# ═══════════════════════════════════════════
# ── KEY GLOBAL: se intenta cargar de Secrets una sola vez ──
_GROQ_KEY_GLOBAL = ""
try:
    _GROQ_KEY_GLOBAL = st.secrets["GROQ_API_KEY"]
except Exception:
    pass

@st.cache_resource
def get_groq_client(key: str = ""):
    """Crear cliente Groq compartido para toda la app."""
    k = key or _GROQ_KEY_GLOBAL
...
CLAUDE_SYSTEM = """Eres el asistente comercial de Ágora Tech Colombia.
...
Pipeline actual: 84 proyectos activos, $8.6B en cotizaciones, 0 contratos cerrados.
Proyectos clave: Nomad 53 (David Conde / Rafael Torres) cierre más cercano; Bosque San Vicente asamblea 2 mayo única con financiamiento; Tiara pasó primer filtro consejo 24 abril.
Patrones de rechazo: adultos mayores (Inzar VI quórum insuficiente), seguridad (El Bosque), precio alto (Hotel Mendoza, 70A).
...
# ═══════════════════════════════════════════
# IA GROQ — gratuito, sin tarjeta de crédito
# ═══════════════════════════════════════════
...
def init_gemini_auto():
    ...
    key_s = st.session_state.get("groq_key_manual","")
    return get_groq_client(key_s) is not None

def init_gemini_key(key: str):
    """Guardar key de Groq en la sesión y verificar que funciona."""
    try:
        ...
        if r:
            st.session_state["groq_key_manual"] = key
...
def ask_gemini(q, ctx=""):
    # Obtener key guardada (session o global)
    key_session = st.session_state.get("groq_key_manual", "")
    client = get_groq_client(key_session)
    if not client:
        return "⚠️ La IA no está configurada. Ve a ⚙️ Configuración y pega la API Key de Groq."
    try:
...
# ═══════════════════════════════════════════
def pg_login():
    # Cargar key desde query params si existe
    try:
        import base64
        _gk = st.query_params.get("gk", "")
        if _gk and not st.session_state.get("groq_key_manual"):
            _k = base64.b64decode(_gk).decode("utf-8")
            if len(_k) > 10:
                st.session_state["groq_key_manual"] = _k
    except Exception:
        pass
    init_gemini_auto()
    ...
    with col2:
        ...
        # ── Componente localStorage para guardar y cargar la API Key ──
        import streamlit.components.v1 as components
        components.html("""
<style>
.ak-box { background:#F6F9FC; border:1.5px solid #E3EAF3; border-radius:10px;
  padding:14px 16px; margin-bottom:14px; }
.ak-box label { font-size:11px; font-weight:700; color:#8BA3BD;
  text-transform:uppercase; letter-spacing:1px; display:block; margin-bottom:6px; }
.ak-row { display:flex; gap:8px; }
.ak-input { flex:1; padding:9px 12px; border:1.5px solid #E3EAF3; border-radius:8px;
  font-family:monospace; font-size:12px; background:white; }
.ak-input:focus { border-color:#00C896; outline:none; }
.ak-btn { background:linear-gradient(135deg,#00C896,#1A9FCC); color:#04111E;
  font-weight:700; border:none; border-radius:8px; padding:9px 16px;
  cursor:pointer; font-size:12px; white-space:nowrap; }
.ak-ok { font-size:10px; color:#009E78; margin-top:5px; display:none; }
</style>
<div class="ak-box">
  <label>⚡ API Key de IA (Groq) — se guarda en este navegador</label>
  <div class="ak-row">
    <input class="ak-input" type="password" id="ak" placeholder="gsk_... (pídela a Luisa)" />
    <button class="ak-btn" onclick="saveKey()">Guardar</button>
  </div>
  <div class="ak-ok" id="ak-ok">✅ Guardada permanentemente en este navegador</div>
</div>
<script>
// Cargar key guardada al abrir
(function(){
  var k = localStorage.getItem('agora_groq');
  if(k){ document.getElementById('ak').value=k;
    document.getElementById('ak-ok').style.display='block'; }
})();
function saveKey(){
  var k = document.getElementById('ak').value.trim();
  if(!k||k.length<10){ alert('Pega la API Key que te dio Luisa'); return; }
  localStorage.setItem('agora_groq', k);
  document.getElementById('ak-ok').style.display='block';
  // Pasar a Streamlit via URL
  var url = new URL(window.location.href);
  url.searchParams.set('gk', btoa(k));
    ...
    with st.sidebar:
        ...
        <div style='background:rgba(0,200,150,.1);border:1px solid rgba(0,200,150,.2);
          ...
          <div style='font-size:10px;color:rgba(255,255,255,.4);margin-top:1px'>
            {u["rol"].capitalize()} · Ágora Tech · {'🟢 IA Claude activa' if ai_ok else '🔴 IA sin configurar'}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # IA Status para todos los usuarios
        ai_ok = get_groq_client() is not None or st.session_state.get("gemini_model") is not None
        if ai_ok:
            st.markdown("""
            <div style='background:rgba(0,200,150,.1);border:1px solid rgba(0,200,150,.2);
                 border-radius:8px;padding:8px 12px;margin-bottom:8px'>
              <div style='font-size:11px;color:#00C896;font-weight:700'>⚡ IA Activa</div>
              <div style='font-size:10px;color:rgba(255,255,255,.4);margin-top:1px'>Llama 3.3 · Groq</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style='background:rgba(232,64,64,.1);border:1px solid rgba(232,64,64,.2);
                 border-radius:8px;padding:8px 12px;margin-bottom:6px'>
              <div style='font-size:11px;color:#E84040;font-weight:700'>🔴 IA sin configurar</div>
            </div>""", unsafe_allow_html=True)
            # Solo Luisa/gerente puede activar la IA manualmente
            if u.get("rol") == "gerente":
                gkey_sb = st.text_input("API Key Groq", type="password",
                                         placeholder="gsk_...",
                                         key="gkey_sb",
                                         help="Solo para activación temporal en esta sesión")
                if gkey_sb:
                    if init_gemini_key(gkey_sb):
                        st.rerun()
            else:
                st.markdown('<div style="font-size:10px;color:rgba(255,255,255,.3);text-align:center">Pide a Luisa activar la IA</div>', unsafe_allow_html=True)
...
def pg_correos():
    ...
    if not st.session_state.gemini_model:
        st.markdown('<div class="al-r">⚠️ <b>La IA no está configurada.</b> Luisa debe ir a Configuración y agregar la API Key de Groq (gratis en console.groq.com, sin tarjeta)</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if st.button("🤖 Generar correo con IA", use_container_width=True):
            if edif_sel == "— Seleccionar —":
                ...
                st.error("La IA no está configurada. Pide a Luisa que agregue la API Key de Claude en Configuración.")
            else:
...
def pg_asistente():
    ...
    if not st.session_state.gemini_model:
        st.markdown('<div class="al-r">⚠️ <b>La IA no está configurada.</b> Ve a Configuración y agrega la API Key de Groq (console.groq.com — gratis, sin tarjeta).</div>', unsafe_allow_html=True)
        return
    ...
    with st.form("prosp_form"):
        ...
        if st.form_submit_button("🤖 Analizar con IA y Guardar", use_container_width=True):
            if not nom_e:
                ...
                st.error("La IA no está configurada. Ve a Configuración.")
            else:
...
def pg_configuracion():
    header("⚙️","Configuración del Sistema","Activa la IA para los comerciales — se guarda en el navegador")

    st.markdown("#### 🤖 Activar IA Groq — Gratis, sin tarjeta")

    ai_ok = init_gemini_auto()
    if ai_ok:
        st.markdown('<div class="al-g">✅ <b>IA activa y funcionando</b> para todos los usuarios de esta sesión.</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="al-r">🔴 <b>IA no configurada.</b> Pega la API Key abajo para activarla.</div>', unsafe_allow_html=True)

    st.markdown("---")

    # Componente HTML con localStorage — la key queda guardada para siempre en el navegador
    st.markdown("#### 🔑 API Key de Groq (se guarda permanentemente en este navegador)")

    groq_html = """
<style>
.groq-box {
    background: #F6F9FC; border: 1.5px solid #E3EAF3; border-radius: 12px;
    padding: 24px; font-family: 'Lato', sans-serif;
}
.groq-box input {
    width: 100%; padding: 12px 16px; border: 1.5px solid #E3EAF3;
    border-radius: 8px; font-size: 14px; font-family: monospace;
    box-sizing: border-box; margin: 8px 0 16px;
    transition: border-color 0.2s;
}
.groq-box input:focus { border-color: #00C896; outline: none; }
.groq-btn {
    background: linear-gradient(135deg, #00C896, #1A9FCC);
    color: #04111E; font-weight: 700; border: none; border-radius: 8px;
    padding: 12px 28px; cursor: pointer; font-size: 14px;
    font-family: 'Sora', sans-serif; transition: all 0.18s;
    width: 100%;
}
.groq-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,200,150,0.4); }
.groq-status { padding: 12px 16px; border-radius: 8px; margin-top: 12px; font-size: 13px; display: none; }
.groq-ok { background: #F0FDF9; border-left: 3px solid #00C896; }
.groq-saved { font-size: 11px; color: #8BA3BD; margin-top: 8px; text-align: center; }
</style>
<div class="groq-box">
  <div style="font-size:13px;color:#4A6580;margin-bottom:4px">
    <b>Pega aquí la API Key de Groq</b> — se guarda en este navegador y nunca se borra
  </div>
  <input type="password" id="groq-key-input" placeholder="gsk_xxxxxxxxxxxxxxxxxxxx" />
  <div id="groq-saved-msg" class="groq-saved"></div>
  <button class="groq-btn" onclick="saveGroqKey()">⚡ Activar IA en este navegador</button>
  <div id="groq-status" class="groq-status"></div>
</div>

<script>
// Cargar key guardada al abrir
window.addEventListener('load', function() {
    var saved = localStorage.getItem('agora_groq_key');
    if (saved) {
        document.getElementById('groq-key-input').value = saved;
        document.getElementById('groq-saved-msg').textContent = '✅ Key guardada en este navegador — activa automáticamente al abrir la app';
    }
});
...
function saveGroqKey() {
    ...
    if (!key || key.length < 10) {
        alert('Pega una API Key válida de Groq (empieza con gsk_...)');
        return;
    }
    // Guardar en localStorage del navegador (permanente)
    localStorage.setItem('agora_groq_key', key);
    document.getElementById('groq-saved-msg').textContent = '✅ Key guardada permanentemente en este navegador';

    // Enviar la key a Streamlit via URL param para activar la IA
    var url = new URL(window.location.href);
    url.searchParams.set('gk', btoa(key));
    window.location.href = url.toString();
}

// Al cargar, si hay key en URL param, activarla
(function() {
    var url = new URL(window.location.href);
    ...
    if (!gk) {
        ...
        var saved = localStorage.getItem('agora_groq_key');
        if (saved) {
            url.searchParams.set('gk', btoa(saved));
            // Solo redirigir si no hay ya un param
        }
    }
})();
</script>
"""
    st.components.v1.html(groq_html, height=220)

    # Leer key desde query params (enviada por el JS de arriba)
    try:
        import base64
        params = st.query_params
        gk_encoded = params.get("gk", "")
        if gk_encoded:
            key_decoded = base64.b64decode(gk_encoded).decode('utf-8')
            if key_decoded and key_decoded not in ['', 'None']:
                result = init_gemini_key(key_decoded)
                if result:
                    st.success("✅ IA activada correctamente para todos los usuarios")
    except Exception:
        pass

    st.markdown("---")
    st.markdown("#### 📋 Pasos para configurar la IA en cada computador (una sola vez)")

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("""
        **1. En tu computador (Luisa):**
        1. Ve a [console.groq.com](https://console.groq.com)
        2. Inicia sesión con Gmail personal
        3. Clic **API Keys** → **Create API Key**
        4. Copia la key `gsk_...`
        5. Pégala arriba y clic **Activar IA**
        6. Queda guardada permanentemente
        """)
    with col2:
        st.markdown("""
        **2. En el computador de cada comercial:**
        1. Abre la app en el navegador
        2. Inicia sesión con su usuario
        3. Ve a **⚙️ Configuración** (solo aparece para gerente)
        4. **O** — Luisa va físicamente a cada PC y activa la IA ahí

        *Una vez activada en un navegador, queda para siempre.*
        """)

    st.markdown("---")
    st.markdown('<div class="al-b">💡 <b>Alternativa más simple:</b> Comparte el link de la app con los comerciales y diles que solo <b>Luisa tiene que abrir la app en su celular o PC</b> y activar la IA desde Configuración. Luego todos los que usen ese mismo navegador ya la tienen.</div>', unsafe_allow_html=True)
...
# MAIN
# ═══════════════════════════════════════════
# Intentar inicializar la IA en CADA renderizado (para todos los usuarios)
init_gemini_auto()

# Leer key desde URL query params (puesta por el JS de localStorage)
try:
    import base64
    _gk = st.query_params.get("gk", "")
    if _gk and not st.session_state.get("groq_key_manual"):
        _key_decoded = base64.b64decode(_gk).decode("utf-8")
        if _key_decoded and len(_key_decoded) > 10:
            st.session_state["groq_key_manual"] = _key_decoded
            st.session_state["gemini_model"] = Groq(api_key=_key_decoded)
except Exception:
    pass
