...
Especialista en: ventas consultivas B2B, automatización de accesos residenciales, cierre de negocios.
Sistema SALTO HomeLok (nube, Bluetooth, PIN, QR, app iOS/Android).
Financiación 100% a 24/36 meses sin intereses. Llave en mano 40 días. Garantía 36 meses equipos.
Precios base: Adecuaciones $18.8M | Lectora vidrio $4.2M | Segunda puerta vidrio $10.8M | CCTV $13.8M | Mantenimiento $966.400+IVA/mes.
Proyectos clave activos: Nomad 53 (David Conde/Rafael) cierre cercano; Bosque San Vicente asamblea 2 mayo; Tiara pasó primer filtro.
Patrón de rechazo más común: adultos mayores — respuesta: teclado PIN físico con relieve, sin necesidad de smartphone.
Responde en español colombiano. Sé específico, accionable y comercialmente estratégico."""

GEMINI_MODEL = "gemini-2.0-flash"

def get_ai_key():
    """Obtener la API Key de Gemini — primero de session_state, luego de secrets."""
...
def activar_ia(key: str):
    """Guardar key de Gemini y verificar."""
    key = key.strip()
    if not key or len(key) < 10:
        st.error("La key debe empezar con AIzaSy...")
        return False
    try:
        genai.configure(api_key=key)
        m = genai.GenerativeModel(model_name=GEMINI_MODEL)
        r = m.generate_content("hola")
        if r:
            st.session_state["gemini_api_key"] = key
            return True
    except Exception as e:
        st.error(f"❌ Key inválida: {str(e)[:150]}")
        return False

def ask_gemini(q, ctx=""):
    key = get_ai_key()
    if not key:
        return "⚠️ La IA no está configurada. Ve a ⚙️ Configuración y pega tu API Key de Gemini (AIzaSy...)."
    try:
        genai.configure(api_key=key)
...
# Inicializar historial de proyectos si no existe
if "historial" not in st.session_state:
    st.session_state.historial = {}  # {nombre_edificio: [{"fecha","usuario","estado","nota"}]}

def agregar_historial(nombre, estado, nota, usuario=""):
    """Agrega una entrada al historial de un edificio."""
    if nombre not in st.session_state.historial:
        st.session_state.historial[nombre] = []
    st.session_state.historial[nombre].append({
        "fecha": datetime.now().strftime("%d %b %Y %H:%M"),
        "usuario": usuario or st.session_state.get("user",{}).get("nombre","Sistema"),
        "estado": estado,
        "nota": nota
    })

def get_historial(nombre):
    return st.session_state.historial.get(nombre, [])
...
ESTADOS=["nuevo","cotizado","negociacion","contrato","financiacion","ejecucion","novedades","entrega","mantenimiento","perdido"]
ESTADO_LABEL={
    "nuevo":"🔵 Lead","cotizado":"🟡 Enviado","negociacion":"🟠 Negoc.",
    "contrato":"📄 Contrato","financiacion":"💰 Financiación",
    "ejecucion":"🔧 En Obra","novedades":"⚠️ Novedades",
    "entrega":"✅ Entregado","mantenimiento":"🔄 Mantenimiento","perdido":"🔴 Perdido"
}
ESTADO_CLS={
    "nuevo":"b-b","cotizado":"b-y","negociacion":"b-p",
    "contrato":"b-g","financiacion":"b-g","ejecucion":"b-g",
    "novedades":"b-r","entrega":"b-g","mantenimiento":"b-g","perdido":"b-r"
}
    ...
    with st.sidebar:
        ...
        <div style='background:rgba(0,200,150,.1);border:1px solid rgba(0,200,150,.2);
          ...
          <div style='font-size:10px;color:rgba(255,255,255,.4);margin-top:1px'>
            {u["rol"].capitalize()} · {"🟢 Gemini IA activa" if ai_ok else "🔴 IA sin configurar — ve a ⚙️"}</div>
        </div>""", unsafe_allow_html=True)
...
def pg_proyectos():
    ...
    for _,r in dff.iterrows():
        ...
        with st.expander(f"🏢  {r['nombre']}   —   {total}   —   {r.get('comercial','')}",expanded=False):
            ...
            if b3.button("📜 Historial",key=f"h_{r.get('id',r['nombre'])}"):
                st.session_state[f"show_hist_{r['nombre']}"] = not st.session_state.get(f"show_hist_{r['nombre']}", False)
                st.rerun()
            # Mostrar historial si está activado
            if st.session_state.get(f"show_hist_{r['nombre']}", False):
                hist = get_historial(r["nombre"])
                if hist:
                    st.markdown("**📜 Historial de seguimiento:**")
                    for h in reversed(hist):
                        st.markdown(f"""
                        <div style='border-left:3px solid #E3EAF3;padding:8px 12px;margin-bottom:6px;background:#F9FAFB;border-radius:0 8px 8px 0'>
                          <div style='font-size:11px;color:#8BA3BD;margin-bottom:3px'>
                            {h["fecha"]} · <b>{h["usuario"]}</b> · {ESTADO_LABEL.get(h["estado"],h["estado"])}
                          </div>
                          <div style='font-size:13px;color:#04111E'>{h["nota"]}</div>
                        </div>""", unsafe_allow_html=True)
                else:
                    st.info("Sin historial de seguimiento aún")
    ...
    with st.form("form_cot"):
        ...
        if st.form_submit_button("💾 Guardar en CRM",use_container_width=True):
            if not nombre: st.error("El nombre es obligatorio")
                ...
                agregar_historial(nombre.upper(), estado, f"Cotización creada — {fc(valor)}")
        ...
        with st.form("upd_form"):
            ...
            if st.form_submit_button("✅ Guardar actualización",use_container_width=True):
                if not nota.strip(): st.error("La nota es obligatoria")
                    ...
                    agregar_historial(sel, nuevo_e, nota)
...
def pg_configuracion():
    ...
    with st.form("config_ia_form"):
        ...
        nueva_key = st.text_input(
            "API Key de Groq",
            placeholder="AIzaSy_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
        ...
        if nueva_key:
            ...
            if n < 30:
                st.warning(f"⚠️ La key tiene solo {n} caracteres — parece incompleta. Una key de Gemini válida tiene ~39 caracteres.")
            else:
        ...
        if guardar:
            ...
            elif not key_limpia.startswith("AIzaSy"):
                st.error("❌ La key debe empezar con 'AIzaSy'. Verifica que la copiaste completa desde Google Cloud.")
            else:
    ...
    if es_g:
        ...
        """)
        st.code('GEMINI_API_KEY = "AIzaSy_tu-key-aqui"', language="toml")
        ...
        c4.metric("IA Groq","🟢 Gemini" if ai_ok else "🔴 Inactiva")
...
def pg_encuestas():
    hdr("📊","Encuesta de Prospecto","Formulario completo de información preliminar — análisis con IA")
    
    st.markdown("""
    <div class="al-b">💡 Completa este formulario al primer contacto con un prospecto. 
    La IA analiza automáticamente la viabilidad y genera la estrategia de venta.</div>
    """, unsafe_allow_html=True)

    with st.form("encuesta_completa"):
        # ─── SECCIÓN 1: DATOS DEL EDIFICIO ───
        st.markdown("### 🏢 1. Datos del Edificio")
        c1,c2 = st.columns(2)
        with c1:
            nom = st.text_input("Nombre del edificio / conjunto *", placeholder="Ej: Edificio Altos del Pino")
            direccion = st.text_input("Dirección completa *", placeholder="Cra 7 No. 45-23, Bogotá")
            barrio = st.text_input("Barrio / Localidad", placeholder="Chapinero, Usaquén...")
        with c2:
            estrato = st.selectbox("Estrato", ["1","2","3","4","5","6"])
            unidades = st.number_input("Número de unidades (aptos/casas) *", min_value=0, value=0, step=1)
            torres = st.number_input("Número de torres / bloques", min_value=1, value=1, step=1)

        st.markdown("### 👤 2. Datos del Contacto")
        c1,c2 = st.columns(2)
        with c1:
            contacto = st.text_input("Nombre completo del contacto *", placeholder="Juan Pérez")
            rol_c = st.selectbox("Rol del contacto *", [
                "Administrador/a",
        ...
        with c2:
            tel = st.text_input("Teléfono / Celular *", placeholder="310 123 4567")
            email_c = st.text_input("Correo electrónico", placeholder="admin@edificio.com")
        como_llego = st.selectbox("¿Cómo llegó el prospecto?", [
            "Referido de otro edificio","Búsqueda en internet","Redes sociales",
            "El comercial lo prospectó","Evento o feria","Otro"
        ])

        st.markdown("### 🔐 3. Situación de Seguridad Actual")
        c1,c2 = st.columns(2)
        with c1:
            tiene_vig = st.radio("¿Tiene vigilancia tradicional?", ["Sí","No"], horizontal=True)
            costo_vig = st.number_input("Costo mensual vigilancia ($/mes)", min_value=0, value=0, step=100_000, format="%d")
            vig_hasta = st.text_input("Contrato de vigilancia vigente hasta", placeholder="Nov 2026")
        with c2:
            incidentes = st.text_area("Incidentes de seguridad recientes", 
                placeholder="Robos, hurtos, ingresos no autorizados, daños...", height=100)
            otras_empresas = st.radio("¿Han cotizado con otras empresas?", ["No","Sí, una","Sí, varias"], horizontal=True)

        st.markdown("### 🚪 4. Infraestructura del Edificio")
        c1,c2,c3 = st.columns(3)
        with c1:
            puertas_peat = st.number_input("Puertas peatonales", min_value=0, value=1, step=1)
            tipo_p_peat = st.selectbox("Tipo puerta principal", ["Vidrio templado","Reja metálica","Puerta metálica","Ya existe"])
        with c2:
            puertas_veh = st.number_input("Puertas vehiculares", min_value=0, value=0, step=1)
            tipo_p_veh = st.selectbox("Tipo puerta vehicular", ["Vidrio templado","Reja metálica","Barrera","Ya existe"])
        with c3:
            ascensores = st.number_input("Ascensores", min_value=0, value=0, step=1)
            tiene_cctv = st.radio("¿Tiene CCTV?", ["No","Sí, básico","Sí, completo"], horizontal=True)

        st.markdown("### 👥 5. Comunidad")
        c1,c2 = st.columns(2)
        with c1:
            adultos_mayores = st.radio("¿Hay adultos mayores o personas con discapacidad?", ["No","Pocos (menos del 20%)","Muchos (más del 20%)"], horizontal=True)
            detalle_am = st.text_area("Detalle sobre adultos mayores / discapacidad", 
                placeholder="Ej: 3 residentes en silla de ruedas, varios mayores de 80 años sin smartphone...", height=80)
        with c2:
            ambiente = st.selectbox("Ambiente de la comunidad para automatizar", [
                "Muy favorable — ya lo aprobaron en asamblea",
                "Favorable — el consejo apoya",
                "Neutral — están evaluando",
                "Resistencia — algunos propietarios en contra",
                "No se ha discutido"
            ])
            prioridad = st.selectbox("Prioridad del prospecto", [
                "🔥 Alta — quieren cerrar pronto",
                "🟡 Media — evaluando opciones",
                "🔵 Baja — en etapa inicial"
            ])

        st.markdown("### 📋 6. Proceso de Decisión")
        c1,c2 = st.columns(2)
        with c1:
            etapa_dec = st.selectbox("Etapa de decisión *", [
                "Recibiendo cotizaciones por mandato de asamblea",
                "El consejo está evaluando propuestas",
                "Explorando opciones — sin decisión formal",
                "No se ha discutido en asamblea",
                "Ya aprobaron — esperan vencimiento vigilancia"
            ])
            fecha_decision = st.text_input("Fecha estimada de decisión", placeholder="Mayo 2026, Jun 2026...")
        with c2:
            com_asignado = st.selectbox("Comercial asignado *", COMS)
            siguiente_paso = st.selectbox("Siguiente paso acordado", [
                "Enviar cotización",
        ...
        notas_com = st.text_area("Notas del comercial / Contexto adicional", 
            placeholder="Todo lo relevante que ayude a entender este prospecto...", height=100)

        submit = st.form_submit_button("🤖 Analizar con IA y Guardar Prospecto", use_container_width=True)

        if submit:
            if not nom or not contacto or not tel:
                ...
                add_proy({
                    ...
                    "nombre": nom.upper(),
                    "comercial": com_asignado,
                    "contacto": f"{contacto} — {rol_c}",
                    "email": email_c,
                    "total": "$0", "totalNum": 0,
                    "cuota24": "$0", "cuota36": "$0", "c24Num": 0, "c36Num": 0,
                    "vig": str(costo_vig), "vigH": vig_hasta,
                    "estado": "nuevo", "etapaOrig": etapa_dec,
                    "version": "v1",
                    "notas": notas_com,
                    "lastUpdate": datetime.now().isoformat(),
                    "lastNote": f"Prospecto registrado — {etapa_dec}",
                    "fecha": datetime.now().strftime("%d %b %Y"),
                    "drive": ""
                })
                agregar_historial(nom.upper(), "nuevo", f"Prospecto registrado por {com_asignado}. {etapa_dec}.")

                # Análisis con IA
                if ai_activa():
                    ahorro = int(costo_vig) - (int(costo_vig) * 0 + 966400) if costo_vig > 966400 else 0
                    prompt = f"""Analiza este prospecto para Ágora Tech Colombia y genera un análisis comercial completo:

EDIFICIO: {nom} | Dirección: {direccion} | Estrato {estrato}
Unidades: {unidades} | Torres: {torres} | Barrio: {barrio}
Contacto: {contacto} ({rol_c}) | Tel: {tel} | Cómo llegó: {como_llego}

SEGURIDAD ACTUAL:
- Vigilancia: {tiene_vig} — {'$'+f'{int(costo_vig):,}'+'/mes hasta '+vig_hasta if tiene_vig=='Sí' else 'No tiene'}
- Incidentes: {incidentes or 'Ninguno reportado'}
- Otras empresas cotizando: {otras_empresas}

INFRAESTRUCTURA:
- Puertas peatonales: {puertas_peat} ({tipo_p_peat})
- Puertas vehiculares: {puertas_veh} ({tipo_p_veh})
- Ascensores: {ascensores} | CCTV: {tiene_cctv}

COMUNIDAD:
- Adultos mayores/discapacidad: {adultos_mayores} — {detalle_am or 'Sin detalles'}
- Ambiente hacia automatización: {ambiente}
- Prioridad: {prioridad}

PROCESO:
- Etapa: {etapa_dec} | Decisión estimada: {fecha_decision}
- Comercial: {com_asignado} | Siguiente paso: {siguiente_paso}
- Notas: {notas_com or 'Ninguna'}

Genera:
## VIABILIDAD DE VENTA: [ALTA 🔥 / MEDIA 🟡 / BAJA 🔵] — con justificación en 2 líneas

## ANÁLISIS FINANCIERO
(Si tiene vigilancia: calcular ahorro mensual, punto de equilibrio, ahorro a 3 años)

## ESTRATEGIA DE VENTA RECOMENDADA
(3-4 acciones concretas, específicas para este edificio)

## OBJECIONES PROBABLES Y CÓMO RESPONDERLAS
(máximo 3 objeciones con respuesta específica)

## ALERTA ADULTOS MAYORES
(Solo si aplica: estrategia específica para superar esta objeción)

## PRÓXIMOS PASOS — ESTA SEMANA
(3 acciones concretas con responsable)

Sin separadores ---. Negrilla para datos clave. Tono ejecutivo comercial."""

                    with st.spinner("🤖 Analizando el prospecto con IA..."):
                        resultado_ia = ask_gemini(prompt)
                    ...
                    agregar_historial(nom.upper(), "nuevo", f"Análisis IA completado: {resultado_ia[:200]}...")
                    
                    st.download_button(
                        "📥 Descargar análisis",
                        data=f"PROSPECTO: {nom}\n\n{resultado_ia}",
                        file_name=f"prospecto_{nom[:20]}_{datetime.now().strftime('%Y%m%d')}.txt"
                    )
                else:
                    st.warning("⚠️ Configura la IA en ⚙️ Configuración para obtener el análisis automático")
                
                st.success(f"✅ Prospecto **{nom}** guardado correctamente en el CRM")
                st.balloons()
