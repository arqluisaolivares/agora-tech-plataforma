...
@st.cache_resource
def get_groq_client():
    """Crear cliente Groq UNA VEZ para toda la app — se comparte entre todos los usuarios."""
    try:
        # 1. Intentar desde Streamlit Secrets (configuración permanente)
        key = st.secrets["GROQ_API_KEY"]
        if key:
            return Groq(api_key=key)
    except Exception:
        pass
    return None  # Sin cliente si no hay key configurada
...
def init_gemini_auto():
    """Verificar si el cliente global de Groq está disponible."""
    # El cliente global ya fue creado con @st.cache_resource
    # Solo verificamos si existe
    client = get_groq_client()
    if client:
        return True
    # Fallback: verificar sesión manual
    if st.session_state.get("gemini_model"):
        return True
    return False

def init_gemini_key(key: str):
    """Inicializar Groq IA con una key manual."""
    try:
        ...
        if r:
            ...
            st.success("✅ IA activa — Llama 3.1 70B (Groq · Gratis)")
            return True
...
def ask_gemini(q, ctx=""):
    # Prioridad 1: cliente global (desde secrets — para todos los usuarios)
    client = get_groq_client()
    # Prioridad 2: cliente de sesión (ingresado manualmente)
    if not client:
        client = st.session_state.get("gemini_model")
    if not client:
        return "⚠️ La IA no está configurada. Pide a Luisa que agregue GROQ_API_KEY en los Secrets de Streamlit Cloud."
    try:
...
def pg_configuracion():
    ...
    st.markdown("#### 🤖 Configurar IA — Groq (100% Gratuito, sin tarjeta)")
    st.markdown('<div class="al-g">✅ La IA usa <b>Llama 3.1 70B vía Groq</b> — completamente gratis, sin tarjeta de crédito. Es más rápido que Gemini y ChatGPT.</div>', unsafe_allow_html=True)

    ai_ok = st.session_state.gemini_model is not None
    st.markdown(f"**Estado actual:** {'🟢 IA activa y funcionando' if ai_ok else '🔴 IA no configurada'}")

    with st.form("groq_form"):
        gkey = st.text_input("API Key de Groq", type="password",
                              placeholder="gsk_...",
                              help="Obtén tu key gratis en: console.groq.com — sin tarjeta")
        if st.form_submit_button("Activar IA Gratuita", use_container_width=True):
            if gkey:
                if init_gemini_key(gkey):
                    st.rerun()

    st.markdown("---")
    st.markdown("#### 📋 Cómo obtener la API Key de Groq (GRATIS, 3 minutos, sin tarjeta)")
    st.markdown("""
    1. Abre **[console.groq.com](https://console.groq.com)** en tu navegador
    2. Haz clic en **"Sign Up"** e inicia sesión con tu Gmail personal
    3. En el menú izquierdo, clic en **"API Keys"**
    4. Clic en **"Create API Key"**
    5. Nombre: `agora-tech` → Clic en **"Submit"**
    6. **Copia la key** que empieza con `gsk_...`
    7. Pégala arriba y haz clic en **"Activar IA Gratuita"**
    """)

    st.markdown("---")
    st.markdown("#### 📋 Para que la IA funcione siempre (una sola vez en Streamlit Cloud)")
    st.markdown("""
    1. Ve a tu app en **share.streamlit.io**
    2. Clic en **⋮ (tres puntos)** → **Settings** → **Secrets**
    3. Pega esto exactamente (con tu key real):
    """)
    st.code('GROQ_API_KEY = "gsk_TU-KEY-AQUI"', language="toml")
    st.markdown("""
    4. Clic **Save** — la app se reinicia en 1 minuto
    5. Desde ese momento **todos los usuarios** tienen IA activa automáticamente y gratis
    """)
